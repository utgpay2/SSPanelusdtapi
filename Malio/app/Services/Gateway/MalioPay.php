...
class MalioPay extends AbstractPayment
{
    public function purchase($request, $response, $args)
    {
        ...
        $user = Auth::getUser();
        if ($type != 'alipay' and $type != 'wechat' and $type != 'token188') {
        ...
        } else if ($type == 'token188') {
            
            $token188 = new Token188();
            $result = $token188->purchase_maliopay($type, $price);
            return $result;
            if ($result['errcode'] == 0) {
                $return = array(
                    'ret' => 1,
    ...
    public function notify($request, $response, $args)
    {
        ...
        switch ($payment_system) {
            ...
            case ('token188'):
                $token188 = new Token188();
                $pid = $token188->notify($request, $response, $args);
                if ($pid) {
                    $this->postPaymentMaliopay($pid, 'Token188' . $pid);
